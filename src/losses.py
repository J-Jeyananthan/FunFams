"""Loss functions for funfam contrastive learning."""
...
class SupConLoss(nn.Module):
    ...
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        log_prob = logits - torch.log(exp_logits.sum(dim=1, keepdim=True).clamp(min=EPS))
...
class ProxyAnchorLoss(nn.Module):
    ...
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        with_pos_proxies = torch.nonzero(P_one_hot.sum(dim=0) > 0, as_tuple=False).squeeze(dim=1)
        num_valid_pos_proxies = with_pos_proxies.numel()
        
        if num_valid_pos_proxies == 0:
            return torch.log(1.0 + N_sim_sum).mean()
        
        pos_term = torch.log(1.0 + P_sim_sum[with_pos_proxies]).sum() / num_valid_pos_proxies
        neg_term = torch.log(1.0 + N_sim_sum).sum() / self.num_classes
        
        return pos_term + neg_term
    
    def __repr__(self) -> str:
        return (f"ProxyAnchorLoss(num_classes={self.num_classes}, "
                f"embedding_dim={self.embedding_dim}, margin={self.margin}, alpha={self.alpha})")
