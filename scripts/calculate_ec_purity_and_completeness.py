...
def calculate_ec_purity_and_completeness(ec_file, funfams_dir):
    ...
    # For completeness/V-measure: expand multi-EC sequences into one entry per EC,
    # consistent with how purity handles multi-EC sequences.
    all_cluster_labels = []
    all_ec_labels = []

    fasta_files = [f for f in os.listdir(funfams_dir) if f.endswith(".faa") or f.endswith(".aln")]

    for cluster_id, filename in enumerate(fasta_files):
        ...
        with open(alignment_path, "r") as alignment_file:
            for line in alignment_file:
                if line.startswith('>'):
                    ...
                    ecs = ec_annotations.get(uniprot_id, [])
                    ec_terms_in_alignment.extend(ecs)
                    for ec in ecs:
                        all_cluster_labels.append(cluster_id)
                        all_ec_labels.append(ec)
    ...
    if all_ec_labels:
        print("Homogeneity:", homogeneity_score(all_ec_labels, all_cluster_labels))
        print("Completeness:", completeness_score(all_ec_labels, all_cluster_labels))
        print("V-measure:", v_measure_score(all_ec_labels, all_cluster_labels))


if __name__ == '__main__':
    calculate_ec_purity_and_completeness()
