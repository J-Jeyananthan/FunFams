...
# -*- coding: utf-8 -*-
"""
ESM2 embedding script — incremental LMDB version.

Generates per-protein (mean-pooled) or per-residue ESM2 embeddings for sequences
in FASTA format and stores them in an LMDB database. Supports resume on interrupt.

Embeddings are taken from the final transformer layer (last_hidden_state), mean-pooled
over residue positions for per-protein mode.

Example:
  python embed-esm2.py \
    --input /path/to/sequences.fasta \
    --output /path/to/embeddings_lmdb \
...
# Set at runtime from model config (e.g. 1280 for 650M, 2560 for 3B)
EMBEDDING_DIM = None
EMBEDDING_DTYPE = np.float16  # half precision


def get_ESM2_model(model_dir):
    print("Loading ESM2 from: {}".format(model_dir))
...
def fasta_stream(fasta_path: Path, split_char: str, id_field: int):
    ...
    with open(fasta_path, 'r') as fasta_f:
        for line in fasta_f:
            if line.startswith('>'):
                ...
                seq_chunks.append(''.join(line.split()).replace("-", ""))
        if current_id is not None:
            seq = ''.join(seq_chunks)
            yield current_id, seq
...
def get_embeddings(seq_path, emb_path, model_dir, split_char, id_field,
                   per_protein, half_precision,
                   max_residues=12000, max_seq_len=1022, max_batch=256,
                   bucket_size=10000, commit_every=20000, map_size_gb=120,
                   sync_every_secs=600):
    global EMBEDDING_DIM

    model, tokenizer = get_ESM2_model(model_dir)
    if half_precision:
        model = model.half()
    ...
    def embed_batch(batch_items):
        ...
        for batch_idx, identifier in enumerate(pdb_ids):
            ...
            # ESM2 tokens: [CLS] residue_1 ... residue_N [EOS] [PAD]...
            # Skip CLS (index 0); slice residues at indices 1..1+s_len
            emb = embedding_repr.last_hidden_state[batch_idx, 1:1 + s_len]
            if per_protein:
                emb = emb.mean(dim=0)
    ...
    def process_bucket(bucket_items):
        ...
        for identifier, raw_seq, seq_len in remaining:
            batch.append((identifier, raw_seq, seq_len))
            n_res_batch += seq_len
    ...
    buffer = []
    stream = fasta_stream(seq_path, split_char, id_field)
...
def create_arg_parser():
    parser = argparse.ArgumentParser(description=(
        'embed-esm2.py creates ESM2 embeddings for sequences in FASTA format. '
        'Embeddings are taken from the final transformer layer (last_hidden_state) '
        'and saved incrementally to LMDB with resume capability. '
        'Example: python embed-esm2.py --input seqs.fasta --output embeddings_lmdb '
        '--model facebook/esm2_t33_650M_UR50D --half 1 --per_protein 1'))

    parser.add_argument('-i', '--input', required=True, type=str,
                        help='Path to a FASTA file containing protein sequences.')
    parser.add_argument('-o', '--output', required=True, type=str,
                        help='Path for saving embeddings as an LMDB directory.')
    parser.add_argument('--model', required=False, type=str,
                        default="facebook/esm2_t33_650M_UR50D",
                        help='HuggingFace repo or local path to ESM2 checkpoint. '
                             'Default: facebook/esm2_t33_650M_UR50D (1280-dim, 33 layers).')
    parser.add_argument('--split_char', type=str, default='!',
                        help="Character for splitting the FASTA header to get the ID. Default: '!'")
    parser.add_argument('--id', type=int, default=0,
                        help='Index of the identifier field after splitting the FASTA header. Default: 0')
    parser.add_argument('--per_protein', type=int, default=1,
                        help='Return mean-pooled per-protein embedding (1, default) or per-residue (0).')
    parser.add_argument('--half', type=int, default=1,
                        help='Use half-precision (fp16). Default: 1')
    parser.add_argument('--max_seq_len', type=int, default=1022,
                        help='Maximum sequence length to embed. ESM2 max is 1022 residues. Default: 1022')
    parser.add_argument('--max_residues', type=int, default=12000,
                        help='Maximum total residues per batch. Default: 12000')
    parser.add_argument('--max_batch', type=int, default=256,
                        help='Maximum sequences per batch. Default: 256')
    parser.add_argument('--bucket_size', type=int, default=10000,
                        help='Number of sequences to buffer and length-sort per bucket. Default: 10000')
    parser.add_argument('--commit_every', type=int, default=20000,
                        help='Number of embeddings to accumulate before LMDB write. Default: 20000')
    parser.add_argument('--map_size_gb', type=int, default=120,
                        help='LMDB map size in GB. Default: 120')
    parser.add_argument('--sync_every_secs', type=int, default=600,
                        help='Seconds between LMDB sync calls. Default: 600')
    return parser
...
def main():
    ...
    half_precision = bool(int(args.half))
